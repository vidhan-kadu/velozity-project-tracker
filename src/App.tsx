...
const ListView = lazy(() =>
  import("./components/list/ListView").then((m) => ({ default: m.ListView })),
);
const TimelineView = lazy(() =>
  import("./components/timeline/TimelineView").then((m) => ({
    ...
    <div className="h-full flex flex-col relative z-10">
      <header className="shrink-0 px-6 py-4 border-b border-slate-700/30">
        ...
        <div className="mt-4 flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
          <nav
            className="flex bg-slate-800/40 rounded-xl p-1 border border-slate-700/30 w-fit"
            role="tablist"
          ...
          >
            {VIEW_CONFIG.map((view) => (
              <button
                ...
                onClick={() =>
                  dispatch({ type: "SET_VIEW", payload: view.key })
                }
      ...
      {/* Content */}
      <main className="flex-1 p-4 md:p-6 overflow-hidden">{renderView()}</main>
    </div>
  );
}
